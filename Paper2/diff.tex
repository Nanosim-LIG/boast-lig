...
%DIF DEL BOAST-old.tex   Fri Apr 28 10:34:16 2017
%DIF ADD BOAST.tex       Fri Apr 28 10:34:25 2017
...
\author{
   \DIFdelbegin %DIFDELCMD < \IEEEauthorblockN{Brice Videau\IEEEauthorrefmark{1}\IEEEauthorrefmark{2}, Kevin Pouget\IEEEauthorrefmark{1}\IEEEauthorrefmark{3}, Luigi Genovese\IEEEauthorrefmark{4}, Thierry Deutsch\IEEEauthorrefmark{4}, Dimitri Komatitsch\IEEEauthorrefmark{6}, Frédéric Desprez\IEEEauthorrefmark{1}\IEEEauthorrefmark{5}, Jean-François Méhaut\IEEEauthorrefmark{1}\IEEEauthorrefmark{3}}%%%
\DIFdelend \DIFaddbegin \IEEEauthorblockN{Brice Videau\IEEEauthorrefmark{1}\IEEEauthorrefmark{2}, Kevin Pouget\IEEEauthorrefmark{1}\IEEEauthorrefmark{3}, Luigi Genovese\IEEEauthorrefmark{4}\IEEEauthorrefmark{3}, Thierry Deutsch\IEEEauthorrefmark{4}\IEEEauthorrefmark{3}, Dimitri Komatitsch\IEEEauthorrefmark{6}, Frédéric Desprez\IEEEauthorrefmark{1}\IEEEauthorrefmark{5}, Jean-François Méhaut\IEEEauthorrefmark{1}\IEEEauthorrefmark{3}}\DIFaddend \\
   ...
   \IEEEauthorblockA{\IEEEauthorrefmark{3}Université Grenoble Alpes}\\
   \IEEEauthorblockA{\IEEEauthorrefmark{4}CEA/INAC/MEM/L\_Sim}\\
   \IEEEauthorblockA{\IEEEauthorrefmark{5}Inria}\\
   \IEEEauthorblockA{\IEEEauthorrefmark{6}Aix Marseille Univ., CNRS, Centrale Marseille, LMA, Marseille, France.}
}
...
architecture. \DIFdelbegin \DIFdel{The }\DIFdelend \DIFaddbegin \DIFadd{This is done using the native OpenCL support for vectorization.
For instance }\emph{\DIFadd{vload16}} \DIFadd{loads 16 consecutive elements from memory and stores
it in a vector.  Here the loaded vector is of type }\emph{\DIFadd{uchar16}} \DIFadd{meaning that it
is a vector of 16 consecutive bytes. In order to do computations without
overflowing, those }\DIFaddend vectors have to be \DIFdelbegin \DIFdel{kept in memory to ensure an efficient
execution.
}\DIFdelend \DIFaddbegin \DIFadd{converted (using }\emph{\DIFadd{convert\_int16}}\DIFadd{) to
a signed and bigger integer type.  Here they are converted to signed 4 bytes
integers, leading to a vector size of 64 bytes.
}

\DIFadd{Once those vector are loaded they can be used in Single Instruction Multiple
Data (SIMD) paradigm. Operations are done component-wise on the vector. For
instance, }\emph{\DIFadd{v22 * (int16)9}} \DIFadd{multiplies each component of the vector by 9.
The same thing goes for the difference operator. Once computed, the result is
clamped and converted back to bytes using }\emph{\DIFadd{convert\_uchar16}}\DIFadd{.
}

\DIFaddend Using vectors of 16 elements, \DIFaddbegin \DIFadd{5 pixels (}\DIFaddend 15 \DIFdelbegin \DIFdel{useful componentsare
}\DIFdelend \DIFaddbegin \DIFadd{components) can be }\DIFaddend simultaneously
loaded and can be used in computation. \DIFaddbegin \DIFadd{Here the developer chose a pixel based
...
of 15. The first 8 components (0-7) are saved using }\emph{\DIFadd{vstore8}} \DIFadd{then the
next 4 (8,9,a,b) using }\emph{\DIFadd{vstore4}} \DIFadd{and so on.
}

\DIFaddend This optimization yields speedups between 1.5 and 6 depending on the image size.
...
kernel laplace(const int width, const int height,
  ...
  int16 res = v22 * (\DIFdelbegin \DIFdel{int}\DIFdelend \DIFaddbegin \DIFadd{int16}\DIFaddend )9 - v11 - v12 - v13 - v21 - v23 - v31 - v32 - v33;
        res = clamp(res, (int16)0, (int16)255);
  uchar res_ = convert_uchar16(res);

  vstore8(res_.s01234567, 0, dst + 3*width*j + 3*5*i);
  \DIFdelbegin \DIFdel{vstore8}\DIFdelend \DIFaddbegin \DIFadd{vstore4}\DIFaddend (res_.s89ab,     0, dst + 3*width*j + 3*5*i + 8);
  \DIFdelbegin \DIFdel{vstore8}\DIFdelend \DIFaddbegin \DIFadd{vstore2}\DIFaddend (res_.scd,       0, dst + 3*width*j + 3*5*i + 12);
...
Figure~\ref{fig:boast_workflow} illustrates the \DIFdelbegin \DIFdel{envisioned }\DIFdelend workflow and program structure.
...
optimizations. \DIFdelbegin \DIFdel{Then the developer selects a kernel configuration and a target language. Those }\DIFdelend \DIFaddbegin \DIFadd{From this set of optimizations, possible languages targets, and
...
\DIFadd{Once this optimization space is designed, the user selects an optimization
strategy, brute force and genetic algorithms are provided by BOAST, or can
design his own. BOAST will then evaluate the different candidates generated by
the optimization program. The candidate's }\DIFaddend parameters define the output source
code that will be generated by BOAST (Step 2). The resulting code source is
then built according to the \DIFdelbegin \DIFdel{user }\DIFdelend specified compiler and options (Step 3). \DIFdelbegin \DIFdel{If input data are available
then the }\DIFdelend \DIFaddbegin \DIFadd{The }\DIFaddend kernel
can be benchmarked and tested for non regression. Based on the results, other
optimizations can be selected (Step 4)\DIFdelbegin \DIFdel{or new optimizations can
be added to the BOAST sources}\DIFdelend .The process can be repeated until a good
candidate is found on the target platform. The resulting kernel is then added
to the program (Step 5).

\DIFdelbegin \DIFdel{Several steps in the workflow can be automated: optimization and compiler flags
exploration,
non regression testing, benchmarking, \ldots This automation can be
scripted by the user or by interfacing other dedicated tools.
In order
to achieve those goals }\DIFdelend \DIFaddbegin \DIFadd{Of course, this workflow if not the only possible use of BOAST.  Especially,
...
\DIFadd{In order too achieve those results }\DIFaddend three aspects should be considered: code
...
The importance of  correct loop imbrication on BLAS~\cite{lawson1979basic} %\FD{BV}{Bizarre la suite ici. Ca fait plus ``previous work''}
...
%%% NON REGRESSION TESTING USING TRACE DEBUGGING %%%
...
global work size increase. This result is \DIFdelbegin \DIFdel{unexpected and understanding why this
happen could be interesting }\DIFdelend \DIFaddbegin \DIFadd{interesting because its unexpected
nature strongly backs the use of autotuning}\DIFaddend . The NVIDIA GPU favors processing
...
%%% CREATING AN AUTO-TUNED CONVOLUTION LIBRARY FOR BIGDFT USING BOAST %%%
...
%%% Porting SPECFEM3D to OpenCL using BOAST %%%
\subsection{Porting SPECFEM3D \DIFaddbegin \DIFadd{Application Kernels: from CUDA }\DIFaddend to OpenCL
using BOAST\label{subsec:specfem}}

In this last subsection, we \DIFdelbegin \DIFdel{study a free seismic wave propagation simulator, SPECFEM3D, in its GLOBE flavor}\footnote{\DIFdel{Specfem3d Globe --- CIG
}%DIFDELCMD < \url{http://www.geodynamics.org/cig/software/specfem3d-globe}%%%
}%DIFAUXCMD
\addtocounter{footnote}{-1}%DIFAUXCMD
\DIFdel{.
}\DIFdelend \DIFaddbegin \DIFadd{present an alternative use-case of the BOAST
...
\DIFadd{The challenges we faced in this case-study were more related to the
complexity of the scientific application and its kernels than to the
BOAST framework.
%DIF > %% SPECFEM3D %%%
}\subsubsection{\DIFadd{SPECFEM3D}}

\DIFaddend SPECFEM3D \DIFaddbegin \DIFadd{GLOBE}\footnote{\DIFadd{SPECFEM3D GLOBE --- CIG
  }\url{http://www.geodynamics.org/cig/software/specfem3d-globe}} \DIFadd{is a
free seismic wave propagation simulator. It }\DIFaddend simulates seismic wave
...
%DIF < %% SPECFEM3D %%%
\DIFdelbegin \subsubsection{\DIFdel{SPECFEM3D}}
%DIFAUXCMD
\addtocounter{subsubsection}{-1}%DIFAUXCMD
%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdelend When we started to work on the project (version v2.1 of July 2013) it
...
some complex kernels took more than 80 parameters and \DIFdelbegin \DIFdel{perform }\DIFdelend \DIFaddbegin \DIFadd{performed }\DIFaddend very
specific physical transformations.

Because of the complexity of \DIFdelbegin \DIFdel{wave propagation (and of the application
architecture), it is hard to impossible to define unit tests}\DIFdelend \DIFaddbegin \DIFadd{the wave propagation kernels, it was
impossible for us to understand the kernel source kernel}\DIFaddend . Hence, \DIFdelbegin \DIFdel{the }\DIFdelend \DIFaddbegin \DIFadd{we
treated them as black-boxes, and aimed at obtaining identical
performances. Furthermore, the kernels' complexity prevents the
specification of unit tests, thus the }\DIFaddend application is validated by the
accuracy of the results it produces \DIFdelbegin \DIFdel{, that is, the accuracy of }\DIFdelend \DIFaddbegin \DIFadd{(the }\DIFaddend seismograms of the simulated
earthquakes\DIFdelbegin \DIFdel{, in comparison with }\DIFdelend \DIFaddbegin \DIFadd{), against }\DIFaddend the actual ones. Non-regression testing (after
...
\DIFdelbegin \DIFdel{As part of the Mont-Blanc project, we had to port SPECFEM3D to OpenCL, so
that it could be used to benchmark Mont-Blanc HPC platforms.
}%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdelend %%% PORTING TO OPENCL %%%
\subsubsection{Porting to OpenCL}

%%%
\paragraph{Porting kernels to BOAST} 
\DIFdelbegin \DIFdel{Nvidia }\DIFdelend \DIFaddbegin \DIFadd{NVidia }\DIFaddend CUDA and OpenCL are based on the same programming model: a
...
CUDA API to OpenCL\DIFdelbegin \DIFdel{API}\DIFdelend \DIFaddbegin \DIFadd{'}\DIFaddend . Most of the functions of the interfaces are very
...
different research teams. On a platform with two K40x GPUs and \DIFdelbegin \DIFdel{N }\DIFdelend \DIFaddbegin \DIFadd{24 }\DIFaddend Intel
Xeon processors, we measured \DIFdelbegin \DIFdel{similar results }\DIFdelend \DIFaddbegin \DIFadd{identical performance }\DIFaddend between the original
CUDA version and our BOAST-CUDA version. \DIFaddbegin \DIFadd{This result was expected, as
the BOAST-generated version is identical to the original, except from
naming differences.
}

\DIFaddend With the same set of optimization flags, BOAST CUDA and OpenCL
\DIFdelbegin \DIFdel{version }\DIFdelend \DIFaddbegin \DIFadd{versions }\DIFaddend reported similar execution time spans. The best execution
...
passed to \DIFaddbegin \DIFadd{the }\DIFaddend OpenCL run-time, as of version 1.1. This parameter, in
...
\DIFdelbegin \DIFdel{By refactoring GPU kernel code in }\DIFdelend \DIFaddbegin \DIFadd{Thanks to the porting of the SPECFEM3D GPU kernels to }\DIFaddend BOAST EDSL, the size of \DIFdelbegin \DIFdel{kernel }\DIFdelend \DIFaddbegin \DIFadd{the
kernels' source }\DIFaddend code shrank by a factor of 1.8 (from 7500 to less than
4000 LOC, \DIFdelbegin \DIFdel{partly
because of code duplication, also removing }\DIFdelend \DIFaddbegin \DIFadd{mainly because of the removal of code duplication and
}\DIFaddend manually unrolled loops). This is beneficial for SPECFEM3D as it
improves the readability and maintainability of its source code. \DIFaddbegin \DIFadd{This
...
try new optimizations in \DIFdelbegin \DIFdel{kernel }\DIFdelend \DIFaddbegin \DIFadd{kernels' }\DIFaddend code and measure their impact,
