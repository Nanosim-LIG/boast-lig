...
texture<float, cudaTextureType1D, cudaReadModeElementType> d_displ_ic_tex;
texture<float, cudaTextureType1D, cudaReadModeElementType> d_accel_ic_tex;
#endif
#ifdef USE_TEXTURES_CONSTANTS
texture<float, cudaTextureType1D, cudaReadModeElementType> d_hprime_xx_ic_tex;
#endif
__device__ void compute_element_ic_att_stress(const int tx, const int working_element, const float * R_xx, const float * R_yy, const float * R_xy, const float * R_xz, const float * R_yz, float * sigma_xx, float * sigma_yy, float * sigma_zz, float * sigma_xy, float * sigma_xz, float * sigma_yz){
  int offset;
  int i_sls;
  float R_xx_val;
  float R_yy_val;
...
__global__ void inner_core_impl_kernel(const int nb_blocks_to_compute, const int NGLOB, const int * d_ibool, const int * d_idoubling, const int * d_phase_ispec_inner, const int num_phase_ispec, const int d_iphase, const float deltat, const int use_mesh_coloring_gpu, const float * d_displ, const float * d_veloc, float * d_accel, const float * d_xix, const float * d_xiy, const float * d_xiz, const float * d_etax, const float * d_etay, const float * d_etaz, const float * d_gammax, const float * d_gammay, const float * d_gammaz, const float * d_hprime_xx, const float * d_hprimewgll_xx, const float * d_wgllwgll_xy, const float * d_wgllwgll_xz, const float * d_wgllwgll_yz, const float * d_kappavstore, const float * d_muvstore, const int COMPUTE_AND_STORE_STRAIN, float * epsilondev_xx, float * epsilondev_yy, float * epsilondev_xy, float * epsilondev_xz, float * epsilondev_yz, float * epsilon_trace_over_3, const int ATTENUATION, const int PARTIAL_PHYS_DISPERSION_ONLY, const int USE_3D_ATTENUATION_ARRAYS, const float * one_minus_sum_beta, const float * factor_common, float * R_xx, float * R_yy, float * R_xy, float * R_xz, float * R_yz, const float * alphaval, const float * betaval, const float * gammaval, const int ANISOTROPY, const float * d_c11store, const float * d_c12store, const float * d_c13store, const float * d_c33store, const float * d_c44store, const int GRAVITY, const float * d_xstore, const float * d_ystore, const float * d_zstore, const float * d_minus_gravity_table, const float * d_minus_deriv_gravity_table, const float * d_density_table, const float * wgll_cube, const int NSPEC_INNER_CORE_STRAIN_ONLY, const int NSPEC_INNER_CORE){
  ...
  if(active){
    ...
    if(GRAVITY){
      compute_element_ic_gravity(tx, working_element, d_ibool, d_xstore, d_ystore, d_zstore, d_minus_gravity_table, d_minus_deriv_gravity_table, d_density_table, wgll_cube, jacobianl, s_dummyx_loc, s_dummyy_loc, s_dummyz_loc,  &sigma_xx,  &sigma_yy,  &sigma_zz,  &sigma_xy,  &sigma_yx,  &sigma_xz,  &sigma_zx,  &sigma_yz,  &sigma_zy,  &rho_s_H1,  &rho_s_H2,  &rho_s_H3);
    }
