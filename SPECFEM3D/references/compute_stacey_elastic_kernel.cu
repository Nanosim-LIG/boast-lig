...
__global__ void compute_stacey_elastic_kernel(realw* veloc,realw* accel,int interface_type,int num_abs_boundary_faces,int* abs_boundary_ispec,int* nkmin_xi, int* nkmin_eta,int* njmin, int* njmax,int* nimin, int* nimax,realw* abs_boundary_normal,realw* abs_boundary_jacobian2D,realw* wgllwgll,int* ibool,realw* rho_vp,realw* rho_vs,int SAVE_FORWARD,realw* b_absorb_field) {
