module BOAST
  ...
  def BOAST::compute_coupling_kernel(ref = true, type)
    ...
    if type == :fluid_CMB then
      ...
      accel_out             = Real("accel_outer_core",          :dir => :out, :dim => [ Dim() ])
      ...
      displ                 = Real("displ_crust_mantle",        :dir => :in,  :dim => [ Dim() ])
      accel_out             = Real("accel_crust_mantle",        :dir => :out, :dim => [ Dim() ])
      ...
      rho_top_oc            = Real("RHO_TOP_OC",                :dir => :in)
      minus_g_cmb           = Real("minus_g_cmb",               :dir => :in)
      gravity               = Int( "GRAVITY")
    ...
    if type == :fluid_ICB or type == :fluid_CMB then
      ...
      variables = [displ, accel_out, accel_in, ibool_2, ibelm_2, normal_outer_core, jacobian2D_outer_core, wgllwgll_xy, ibool_1, ibelm_1, rho_top_oc, minus_g_cmb, gravity, nspec2D]
    end
    ...
    if(get_lang == CUDA and ref) then
      ...
      print If( iface < nspec2D ) {
        ...
        displ_a.each_index { |indx| print displ_a[indx] === displ[iglob_1*3+indx] } unless type == :CMB_fluid
        n.each_index { |indx| print n[indx] === normal_outer_core[INDEX4(ndim,ngllx,ngllx,indx,i,j,iface)] }
        print displ_n === displ_a[0]*n[0] + displ_a[1]*n[1] + displ_a[2]*n[2] unless type == :CMB_fluid
        ...
        elsif type == :fluid_ICB
          print atomicAdd(accel_out+iglob_2, -weight*displ_n)
        elsif type == :CMB_fluid
          print If( gravity ) {
            print pressure === rho_top_oc * ( minus_g_cmb * ( displ[iglob_2*3]*n[0] + displ[iglob_2*3+1]*n[1] + displ[iglob_2*3+2]*n[2] ) - accel_in[iglob_1] )
          }
          print Else() {
            print pressure === -rho_top_oc * accel_in[iglob_1]
          }
