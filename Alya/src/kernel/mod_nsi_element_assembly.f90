module mod_nsi_element_assembly
  ...
  use def_nastin, only :  kfl_nota1_nsi,kfl_limit_nsi
  ...
  public :: nsi_element_assembly_split_oss    ! Assembly using split OSS
...
contains
  ...
  subroutine nsi_element_assembly_asgs_oss(             &
    ...
    real(rp),    intent(inout) :: gpsp2(VECTOR_SIZE,pgaus)
...
#define DEF_VECT 1:VECTOR_SIZE
       ...
       if( ndime == 2 ) then
          ...
          do igaus = 1,pgaus
             ...
             do inode = 1,pnode
                ...
                !              facx1 = factx(DEF_VECT) * gpsha(DEF_VECT,inode,igaus)
                !              facy1 = facty(DEF_VECT) * gpsha(DEF_VECT,inode,igaus)         
                do jnode = 1,pnode              
                   !
                   ...
                   elauu11(DEF_VECT,inode,jnode) = elauu11(DEF_VECT,inode,jnode) - facx1 * gpcar(DEF_VECT,1,jnode,igaus)                                                             ! Auu_xx
                   elauu12(DEF_VECT,inode,jnode) = elauu12(DEF_VECT,inode,jnode) - facx1 * gpcar(DEF_VECT,2,jnode,igaus)                                                             ! Auu_xy
                   elauu21(DEF_VECT,inode,jnode) = elauu21(DEF_VECT,inode,jnode) - facy1 * gpcar(DEF_VECT,1,jnode,igaus)                                                             ! Auu_yx
                   elauu22(DEF_VECT,inode,jnode) = elauu22(DEF_VECT,inode,jnode) - facy1 * gpcar(DEF_VECT,2,jnode,igaus)                                                             ! Auu_yy
                   !
    ...
    if (kfl_regim_nsi == 3 .and. kfl_confi_nsi == 1) then ! Penalization of pressure, never imposse pressure
       do igaus =1, pgaus
          penal = 1.0e-4_rp*gpden(DEF_VECT,igaus)/gpvis(DEF_VECT,igaus)
          do inode =1, pnode
             fact0(DEF_VECT) = penal*p2sca(DEF_VECT,inode,igaus)
    ...
    do igaus = 1,pgaus
       ...
       do inode = 1,pnode
          ...
          elrbp(DEF_VECT,inode) = elrbp(DEF_VECT,inode) + gprhc(DEF_VECT,igaus) * p2sca(DEF_VECT,inode,igaus)                       ! ( rhs , q)
       end do
    end do
    !
    ! Newton-Raphson
    ...
    do ivect = 1,VECTOR_SIZE
       rbubl(ivect) = real(min(1_ip,pbubl(ivect)),rp)
    end do
...
#define DEF_VECT 1:VECTOR_SIZE
  ...
  subroutine nsi_element_assembly_asgs_oss_old(         &
    ...
    integer(ip)                :: idof1,idof2,idof3,ivect
...
!!$  end if
          ...
          do igaus = 1,pgaus
             ...
             do inode = 1,pnode
                ...
                do jnode = 1,pnode              
                   ...
                   !                                               + (11)   - 2/3 * B * ( mu d(div u)/dxi , vi )
                   !
                   elauu(DEF_VECT,idof1,jdof1) = elauu(DEF_VECT,idof1,jdof1) + fact1(DEF_VECT) * gpcar(DEF_VECT,1,jnode,igaus) + fact0 * gphes(DEF_VECT,1,jnode,igaus)  ! Auu_xx
                   elauu(DEF_VECT,idof1,jdof2) = elauu(DEF_VECT,idof1,jdof2) + fact2(DEF_VECT) * gpcar(DEF_VECT,1,jnode,igaus) + fact0 * gphes(DEF_VECT,3,jnode,igaus)  ! Auu_xy
                   elauu(DEF_VECT,idof2,jdof1) = elauu(DEF_VECT,idof2,jdof1) + fact1(DEF_VECT) * gpcar(DEF_VECT,2,jnode,igaus) + fact0 * gphes(DEF_VECT,3,jnode,igaus)  ! Auu_yx
                   elauu(DEF_VECT,idof2,jdof2) = elauu(DEF_VECT,idof2,jdof2) + fact2(DEF_VECT) * gpcar(DEF_VECT,2,jnode,igaus) + fact0 * gphes(DEF_VECT,2,jnode,igaus)  ! Auu_yy
          ...
          do igaus = 1,pgaus
             ...
             do inode = 1,pnode
                ...
                do jnode = 1,pnode              
                   ...
                   fact1(DEF_VECT) = (gpadv(DEF_VECT,1,igaus) * gpcar(DEF_VECT,1,inode,igaus) + gpadv(DEF_VECT,2,igaus) * gpcar(DEF_VECT,2,inode,igaus) + gpadv(DEF_VECT,3,igaus) * gpcar(DEF_VECT,3,inode,igaus)) * gpsha(DEF_VECT,jnode,igaus) * fact0(DEF_VECT)
                   fact2(DEF_VECT) = (gpadv(DEF_VECT,1,igaus) * gpcar(DEF_VECT,1,jnode,igaus) + gpadv(DEF_VECT,2,igaus) * gpcar(DEF_VECT,2,jnode,igaus) + gpadv(DEF_VECT,3,igaus) * gpcar(DEF_VECT,3,jnode,igaus)) * gpsha(DEF_VECT,inode,igaus) * fact0(DEF_VECT)
          ...
          do igaus = 1,pgaus
             ...
             do inode = 1,pnode
                ...
                do jnode = 1,pnode
                   ...
                   ! fact4 = - b * ( rho * div(a) u , v ) 
                   !
                   fact2(DEF_VECT) = ( gpadv(DEF_VECT,1,igaus) * gpcar(DEF_VECT,1,inode,igaus) + gpadv(DEF_VECT,2,igaus) * gpcar(DEF_VECT,2,inode,igaus) + gpadv(DEF_VECT,3,igaus) * gpcar(DEF_VECT,3,inode,igaus)) * gpsha(DEF_VECT,jnode,igaus)
                   fact3(DEF_VECT) = ( gpadv(DEF_VECT,1,igaus) * gpcar(DEF_VECT,1,jnode,igaus) + gpadv(DEF_VECT,2,igaus) * gpcar(DEF_VECT,2,jnode,igaus) + gpadv(DEF_VECT,3,igaus) * gpcar(DEF_VECT,3,jnode,igaus)) * gpsha(DEF_VECT,inode,igaus)
                   fact4(DEF_VECT) = fact5(DEF_VECT) * gpsha(DEF_VECT,inode,igaus) * gpsha(DEF_VECT,jnode,igaus)
                   fact2(DEF_VECT) = fact0(DEF_VECT) * fact2(DEF_VECT)
                   fact3(DEF_VECT) = fact0(DEF_VECT) * fact3(DEF_VECT)
                   fact6(DEF_VECT) = fact2(DEF_VECT) + fact3(DEF_VECT) + fact4(DEF_VECT)
          ...
          do igaus = 1,pgaus
             do inode = 1,pnode
                ...
                do jnode = 1,pnode
                   ...
                   jdof2           = 3*jnode-1
                   jdof3           = 3*jnode                
                   fact1(DEF_VECT) = fact0 * gpsha(DEF_VECT,jnode,igaus)
                   ! Auu_1i
                   elauu(DEF_VECT,idof1,jdof1) = elauu(DEF_VECT,idof1,jdof1) + fact1(DEF_VECT) * gpgve(DEF_VECT,1,1,igaus) ! rho * ux * dux^i-1/dx
                   elauu(DEF_VECT,idof1,jdof2) = elauu(DEF_VECT,idof1,jdof2) + fact1(DEF_VECT) * gpgve(DEF_VECT,2,1,igaus) ! rho * uy * dux^i-1/dy
    ...
    if( kfl_linea_nsi == 2 ) then
       if( ndime == 2 ) then
          do igaus = 1,pgaus
             fact0 =  gpden(DEF_VECT,igaus) * gpvol(DEF_VECT,igaus) 
             do inode = 1,pnode
                fact1 = fact0 * gpsha(DEF_VECT,inode,igaus)
                elrbu(DEF_VECT,1,inode) = elrbu(DEF_VECT,1,inode) + fact1 * ( gpadv(DEF_VECT,1,igaus) * gpgve(DEF_VECT,1,1,igaus) + gpadv(DEF_VECT,2,igaus) * gpgve(DEF_VECT,2,1,igaus) )
                elrbu(DEF_VECT,2,inode) = elrbu(DEF_VECT,2,inode) + fact1 * ( gpadv(DEF_VECT,1,igaus) * gpgve(DEF_VECT,1,2,igaus) + gpadv(DEF_VECT,2,igaus) * gpgve(DEF_VECT,2,2,igaus) )
             end do
          end do
       else
          do igaus = 1,pgaus
