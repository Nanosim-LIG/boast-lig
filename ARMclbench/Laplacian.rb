...
puts optimizer.optimize { |opt|
  ...
  sizes.each_index { |indx|
    ...
    time_per_pixel=[]
    (0..3).each {
      stats = k.run(input_buff, output_buff, width, height, :global_work_size => [rndup((width*3/opt[:x_component_number].to_f).ceil,opt[:local_work_size][0]), rndup((height/opt[:y_component_number].to_f).ceil, opt[:local_work_size][1]), opt[:local_work_size][2]], :local_work_size => opt[:local_work_size])
