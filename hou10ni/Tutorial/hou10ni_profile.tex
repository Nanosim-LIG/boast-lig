...
This document presents a study of hou10ni's kernel. We present results obtained with the performance evaluation tools TAU and Paje on the application.
...
\section{Build and launch hou10ni}
...
\noindent Execution with 4 MPI processes:
\begin{cverbatim}
mpirun -np 4 build/hou10ni.out < HPC4E.dat
\end{cverbatim}
...
\begin{cverbatim}
export TAU_MAKEFILE=$TAU_DIR/Makefile.tau...  (ex.: Makefile.tau-icpc-mpi-pdt)
...
\noindent $\rightarrow$ Press "t" to change the compiler and chose \texttt{tau\_f90.sh} and add TAU lib\\
 (e.g., \texttt{/home/esaillard/TOOLS/TAU/tau-2.26/build/x86\_64/lib/libTAU\_traceinput-icpc-mpi-pdt.a}) in the linker flag. Then you can tape \texttt{make} and launch the application.

\begin{cverbatim}
make
cd .. 
mpirun -np 4 tau_exec -T mpi  -io -memory build/hou10ni.out < HPC4E.dat
\end{cverbatim}
...
\section{Trace Analysis with PAJE}
...
We generate the same kernel as a reference version (KRef ) and run it with BOAST in order to compare the results in term of correctness and performances. This version simply needs to keep the Fortran code (referred as \texttt{codeF}) and wrap it with BOAST. A reference version should follow the template bellow.
...
def KRef
  ...
  kernel_name = "function_ref"

  # ARGUMENTS OF KERNEL HERE
  # FOR EXAMPLE : 
  geocode = BOAST::Int("geocode", :dir => :in )
  u = BOAST::Real("u", :dir => :in, :dim => [ BOAST::Dim(0, n01),  BOAST::Dim(0, n02))] )
 
   p = BOAST::Procedure::new(kernel_name, [geocode,u])
