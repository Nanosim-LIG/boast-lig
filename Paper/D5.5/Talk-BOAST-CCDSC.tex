...
\author[B. V.]{\textbf{\textbf{Fr\'ed\'eric~Desprez}}~\inst{1}, Brice~Videau~\inst{1,3}, Kevin~Pouget~\inst{1}, Luigi~Genovese~\inst{2},
...
\date{\textbf{Workshop CCDSC}\\October 6, 2016}
...
\begin{frame}
  ...
  \begin{itemize}
    \item Ad hoc autotuners (usually for libraries):
    \begin{itemize}
      \item Atlas~\cite{whaley04} (C macro processing)
      \item SPIRAL~\cite{puschel2004spiral} (DSL)
      \item ...
    \end{itemize}
    \item Generic frameworks using annotation systems:
    \begin{itemize}
      \item POET~\cite{yi2007poet} (external annotation file)
      \item Orio~\cite{Hart2009:Orio} (source annotation)
      \item BEAST~\cite{CPE:CPE3516} (Python preprocessor based, embedded DSL for optimization space definition/pruning)
    \end{itemize}
    \item Generic frameworks using embedded DSL:
    \begin{itemize}
      \item Halide~\cite{ragan2013halide} (C++, not very generic, 2D stencil targeted)
      \item Heterogeneous Programming Library~\cite{F.Fabeiro:2016:WPM:2894387.2894576} (C++)
    \end{itemize}
  \end{itemize}
