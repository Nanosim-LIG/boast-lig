...
SUBROUTINE magicfilter(n, ndat, x, y)
  integer(kind=4) :: n
  integer(kind=4) :: ndat
  real(kind=8), intent(in), dimension(0:n - (1), 1:ndat + 1 - (1)) :: x
  real(kind=8), intent(out), dimension(1:ndat + 1 - (1), 0:n - (1)) :: y
...
\begin{lstlisting}
void magicfilter(int32_t n, int32_t ndat, const double * x, double * y){
}
\end{lstlisting}
...
\emph{n} and \emph{ndat}. We can see here the use of the \emph{open} and
