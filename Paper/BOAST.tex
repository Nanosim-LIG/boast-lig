...
\lstdefinestyle{BOAST}{
  ...
  frame=single, 
  captionpos=b,
  morekeywords={BOAST, print, decl, close}
}
...
related to manipulations in the host language three keywords were defined. They
are \emph{print}, \emph{decl} and \emph{close}. As the language is an EDSL,
these three keywords are methods in the BOAST namespace.

The \emph{print} method calls the public \emph{print} method of Objects it is
...
The \emph{decl} method is used for two purposes. First, it can be used to
declares a variable. Second, it can be used to print the beginning of a control
structure without an associated code block.

The \emph{close} method is the counterpart to the \emph{decl} method. It is used
...
code. These classes can be sorted in two groups:
...
\begin{lstlisting}
i = BOAST::Int("i") # or BOAST::Variable::new(Int, "i")
k = BOAST::Int("k", :size => 8)
l = BOAST::Real("l", :dim => [ BOAST::Dim(-5, 20) ], :local => true )
BOAST::decl i, k, l
BOAST::print i === 5
j = i + 5
BOAST::print k === j * 2
...
while the classical assignment is kept as the \emph{=} operator.

Figure~\ref{fig:BOAST_algebra} shows some basic usage of both Variables and
Expressions as well as the \emph{print} and \emph{decl} keywords. For clarity we
stayed out of BOAST namespace so BOAST related class and methods are prefixed by
\emph{BOAST::}. Listing~\ref{lst:BOAST_algebra} shows the BOAST code that when
run produces the FORTRAN (Listing~\ref{lst:BOAST_algebra_FORTRAN}) and C
(Listing~\ref{lst:BOAST_algebra_C}) output.
