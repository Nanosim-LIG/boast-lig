...
\lstdefinestyle{BOAST}{
  ...
  frame=single, 
  captionpos=b,
  morekeywords={BOAST, print, decl, open, close}
}
...
are \emph{print}, \emph{decl}, \emph{open} and \emph{close}. As the language is
...
The \emph{open} method can be used to print the beginning of a control
structure without an associated code block.

The \emph{close} method is the counterpart to the \emph{open} method. It is used
...
BOAST::print k === j * 2
BOAST::print l[k] === 1.0
BOAST::print l[k-1] === BOAST::sin(l[k])
\end{lstlisting}
...
(Listing~\ref{lst:BOAST_algebra_C}) output. On the last line, a call to the
\emph{sin} function is made through the automatic creation of a FuncCall object.
This automatic behaviors is limited to BOAST namespace, and thus, here, the
function call has to be prefixed with \emph{BOAST::}.
...
The classical control structures are implemented. If, For, While, Case are
abstractions in BOAST matching the behavior of corresponding control structures
in other languages. An exception is the For in BOAST more closely matches the
for in FORTRAN than the one in C.
