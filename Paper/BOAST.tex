...
metaprogramming framework dedicated computing kernels.  BOAST allows the
description of a kernel and its possible optimizations using a DSL.  BOAST
runtime will then compare the different version's performance as well as verify
...
\begin{keyword}
Code Generation \sep Portability \sep Genericity \sep Autotuning \sep Non
Regression Testing \sep Productivity and Software Design \sep High Performance
Computing
\end{keyword}
...
and low levels don't avoid the need of optimization which becomes more and more
unavoidable with new architectures based on many cores and accelerators.

Unfortunately, portability efforts of the low level i.e. computing kernels are
often lost when migrating to a new architecture. Worse, code may lose
maintainability because several versions of some functionalities coexist,
...
test automatically the many possibilities of optimizations and choose the best
one after validation from a reference version.  Then BOAST can generate pieces
of Fortran, C, OpenCL, or CUDA codes, test it for a specific target
architecture.
...
be quite costly and degrades performance as optimization opportunities that
...
The paper is organized as follows. First we further motivate the need for
metaprogramming frameworks.  Secondly we illustrate the optimization problems
related to scientific computing with the Laplace equation. Thirdly we present
our solution, called BOAST, its architecture and its functionalities. Fourthly
we apply BOAST to three use cases: the Laplace equation motivating example;
short multi-dimensional convolutions from the BigDFT quantum chemistry
application; and the seismic wave propagation simulator SPECFEM3D where 80 GPU
computing kernels have been ported improving, considerably the maintainability
and providing a single code base for OpenCL and CUDA kernel versions. Finally
we compare BOAST with other related works.
