...
#include "convolut_kinetic_per_c_k.h"
...
main()
{
    ...
    // Create input
    FILE *fpx1 = fopen("input_c_x1", "r");
    FILE *fpx2 = fopen("input_c_x2", "r");
    FILE *fpy1 = fopen("input_c_y1", "r");
    FILE *fpy2 = fopen("input_c_y2", "r");
    ...
    for (int j = 0; j < n3; ++j)
        for (int i = 0; i < n2; ++i)
            for (int i1 = 0; i1 < n1; ++i1)
            {
                ...
                fscanf(fpx1, "%lf\n",
                       &t3[OFFSET_OF_OUTPUT(i1, i, j, n2, n1)]);
                fscanf(fpx2, "%lf\n",
                       &t4[OFFSET_OF_OUTPUT(i1, i, j, n2, n1)]);
                fscanf(fpy1, "%lf\n",
                       &t1[OFFSET_OF_OUTPUT(i1, i, j, n2, n1)]);
                fscanf(fpy2, "%lf\n",
                       &t2[OFFSET_OF_OUTPUT(i1, i, j, n2, n1)]);
            }
#endif
    ...
    convolut_kinetic_per_c_k_wrapper(n1, n2, n3, hgrid, t3, t4, y1, y2, 
                                     c_in, k1, k2, k3);
    ...
    for (int j = 0; j < (n3); ++j)
        for (int i = 0; i < (n2); ++i)
            for (int i1 = 0; i1 < (n1); ++i1)
                printf("%24.20f\n", y1[OFFSET_OF_OUTPUT(i1, i, j, (n2), (n1))]);
#endif
    ...
    double error_y2 = 0.0;
    {
        ...
        FILE *fpy1t = fopen("output_c_y1", "r");
        FILE *fpy2t = fopen("output_c_y2", "r");
        ...
        for (int j = 0; j < (n3); ++j)
            for (int i = 0; i < (n2); ++i)
                for (int i1 = 0; i1 < (n1); ++i1)
                {
                    fscanf(fpy1t, "%lf\n", &y1t);
                    fscanf(fpy2t, "%lf\n", &y2t);
                    error_y2 += fabs(y2[OFFSET_OF_OUTPUT(i1, i, j, (n2), (n1))]-y2t);
                    error_y1 += fabs(y1[OFFSET_OF_OUTPUT(i1, i, j, (n2), (n1))]-y1t);
                    if (fabs(y1[OFFSET_OF_OUTPUT(i1, i, j, (n2), (n1))]-y1t) > fabs(y1t*0.1))
                    {
                        printf("Error at (%d, %d, %d): %24.20f %24.20f\n", i1, i, j, y1t, y1[OFFSET_OF_OUTPUT(i1, i, j, (n2), (n3))]);
                        return -1;
                    }
                    if (fabs(y2[OFFSET_OF_OUTPUT(i1, i, j, (n2), (n1))]-y2t) > fabs(y2t*0.1))
                    {
