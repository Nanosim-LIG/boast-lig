...
program conv_check
  ...
  integer :: i,i_stat,i_all,j,i1,i2,i3,ntimes,ndat,i1_max,i_max,it0,it1,itimes
  integer :: count_rate,count_max,l,ierror,i1s,i1e
  integer :: nvctr_cf,nseg,iseg
  real(wp) :: tt,scale
...
!  call init_thread_engine();
           ...
           allocate(psi_cuda(n2*n3,n1,1+ndebug),stat=i_stat)
...
!           call nanosec(tsc0);
!           do i=1,ntimes
!              call my_convrot_n_per(n1-1,ndat,psi_in,psi_out)
!           end do
!           call nanosec(tsc1);
!           !call system_clock(it1,count_rate,count_max)
!
...
!           call nanosec(tsc0);
           do i=1,ntimes
              call magicfilter1d_naive(n1,n2*n3,psi_in,psi_cuda)
           end do
!           call nanosec(tsc1);
...
subroutine my_convrot_n_per(n1,ndat,x,y)
  use module_base
  implicit none
...
END SUBROUTINE my_convrot_n_per
...
subroutine convrot_n_per_3d_transpose(n1,n2,n3,x,y,tmp)
  ...
  call my_convrot_n_per(n1-1,n2*n3,x,y)
  call my_convrot_n_per(n2-1,n1*n3,y,tmp)
  call my_convrot_n_per(n3-1,n1*n2,tmp,y)
